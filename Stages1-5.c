...
int execute(char input[MAXBUFFSIZE]){
    ...
    switch(cmdID){
        ...
        case 8:
            return invoke_command_negative(Tokens.tokens[0]);
...
int invoke_command_negative(char* input) {
    ...
    if(is_history_empty()){
        ...
        index = (history.next + no_commands - num_arg - 1) % SIZEOFHISTORY;
    
    else
        index = history.next - num_arg - 1;
